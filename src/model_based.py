...
def gather_random_trajectories(num_traj, env_name):
    ...
    # print some stats
    print('Mean R:', np.round(np.sum(game_rewards)/num_traj, 2), 'Max R:',
          np.round(np.max(game_rewards), 2), np.round(len(game_rewards)/num_traj))
...
def train_dyna_model(random_dataset, rl_dataset, env_model, rew_model, batch_size, max_model_iter, num_examples_added, ENV_LEARNING_RATE, REW_LEARNING_RATE, device):
    ...
    X_train = np.array([np.concatenate([obs, act])
                        for obs, _, _, _, act in D_train])  # Takes obs and action
    # Reward's output
    y_rew_train = np.array([[rw] for _, _, rw, _, _ in D_train])
    # Next state output
    y_env_train = np.array([no for _, no, _, _, _ in D_train])
    # y(state) = s(t+1) - s(t)
    y_env_train = y_env_train - np.array([obs for obs, _, _, _, _ in D_train])

    # Create the input and output array for the validation
    X_valid = np.array([np.concatenate([obs, act])
                        for obs, _, _, _, act in D_valid])  # Takes obs and action
    # Reward output
    y_rew_valid = np.array([[rw] for _, _, rw, _, _ in D_valid])
    # Next state output
    y_env_valid = np.array([no for _, no, _, _, _ in D_valid])
    # y(state) = s(t+1) - s(t)
    y_env_valid = y_env_valid - np.array([obs for obs, _, _, _, _ in D_valid])
    ...
    for it in tqdm(range(max_model_iter)):
        ...
        if it % 10 == 0:
          ...
          print('..', it, valid_env_loss.cpu().detach().numpy(),
                valid_rew_loss.cpu().detach().numpy())
...
def multi_model_based_control(env_model, rew_model, real_obs, num_sequences, horizon_length, sample_action, norm, device):
    ...
    for t in range(horizon_length):
      ...
      # scale the input
      models_input = input_scaler.transform(
          np.concatenate([m_obs, sampled_actions], axis=1))
      ...
      pred_obs = env_output_scaler.inverse_transform(
          pred_obs.cpu().detach().numpy())
...
date_time = "{}_{}.{}".format(now.day, now.hour, now.minute)
...
if __name__ == '__main__':
    ...
    if save_video_test:
        env = gym.wrappers.Monitor(
            env,  "VIDEOS/TEST_VIDEOS_"+writer_name, video_callable=lambda episode_id: True)
    obs = env.reset()
    ...
    env_model = NNDynamicModel(
        env.action_space.shape[0] + env.observation_space.shape[0], env.observation_space.shape[0]).to(device)
    rew_model = NNRewardModel(
        env.action_space.shape[0] + env.observation_space.shape[0], 1).to(device)
    ...
    for n_iter in range(AGGR_ITER):
        ...
        train_env_loss, train_rew_loss, valid_env_loss, valid_rew_loss, norm = train_dyna_model(
            rand_dataset, rl_dataset, env_model, rew_model, BATCH_SIZE, TRAIN_ITER_MODEL, num_examples_added, ENV_LEARNING_RATE, REW_LEARNING_RATE, device)
        print('{} >> Eloss:{:.4f} EV loss:{:.4f} -- Rloss:{:.4f} RV loss:{:.4f}'.format(
            n_iter, train_env_loss, valid_env_loss, train_rew_loss, valid_rew_loss))
        ...
        while num_examples_added < STEPS_PER_AGGR:
            while True:
                ...
                action, pred_rew = multi_model_based_control(
                    env_model, rew_model, obs, NUM_ACTIONS_SEQUENCES, HORIZION_LENGTH, env.action_space.sample, norm, device)
                ...
                rew_model.train(True)
                unnorm_rew = rew_output_scaler.inverse_transform(
                    [float(p_rew.cpu().data[0])]).squeeze()
                ...
                if done:
                    obs = env.reset()
                    print('  >> R: {:.2f}, Mean sum:{:.2f}, {}'.format(
                        game_reward, np.mean(game_pred_rews), num_examples_added))
        ...
        torch.save({
            "Dynamic":env_model.state_dict(),
            "Reward":rew_model.state_dict()
            },
            os.path.join(
            log_dir, ENV_NAME, str(args.num), str(n_iter)+"_"+str(np.mean(rews)) + ".pth.tar"))
        print("Saving at {}".format(os.path.join(
            log_dir,ENV_NAME,str(args.num),str(n_iter)+"_"+str(np.mean(rews)) + ".pth.tar")))
